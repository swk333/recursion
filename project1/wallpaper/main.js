...
class EmotionObject{
  ...
  //感情のすべての擬音語のWord オブジェクトの配列を返します
  static getOnomatopoeiaWords(onoArr){
    let rtnArr = [];
    for(let i = 0; i < onoArr.length; i++) {
      let wrd = new Word(onoArr[i], dictionary[onoArr[i]], pictureDictionary[onoArr[i]]);
      rtnArr.push(wrd);
    }
    return rtnArr;
  }
  ...
  getHtmlContainerString(){
    let detailArr = EmotionObject.getOnomatopoeiaWords(this.onomatopoeia);
    let backgroundDiv = 
    `
      <div class='vw-100 p-3' style="background-color:${this.color}">
        <div class="text-white" id="${this.emotion}">
          <h3>${this.emotion}</h3>
    ...
    for(let i = 0; i < detailArr.length; i++){
      explainDiv += 
      `
          <div class="m-2 p-2 col-5 d-flex justify-content-spacebetween bg-white">
            <div>
              <p>${detailArr[i]["word"]}</p>
              <p>${detailArr[i]["defintion"]}</p>
            </div>
            <div>
              <img class="img" src=${detailArr[i]["pictureUrl"]}>
            </div>
          </div>

      `
    }
    let htmlString = `${backgroundDiv} ${explainDiv} </div></div></div>`;
  return htmlString;
  }
...
function renderCategory(emotionObject) {
  const target = document.getElementById("target");
  const cardDiv = document.createElement("div");
  const emotionNameDiv = document.createElement("div");
  const imageDiv = document.createElement("div");
  const idUrl = document.createElement("a");
  cardDiv.classList.add("col-3", "p-3", "text-white", "d-flex", "justify-content-center", "flex-column", "align-items-center", "m-2");
  imageDiv.classList.add("p-1");
  cardDiv.style.backgroundColor = emotionObject["color"];
  emotionNameDiv.innerHTML = `<a href="#${emotionObject["emotion"]}">${emotionObject["emotion"]}</a>`
  cardDiv.append(emotionNameDiv);
  target.append(cardDiv);

}

for(let i = 0; i < emotions.length; i++){
  renderCategory(emotions[i]);
}
